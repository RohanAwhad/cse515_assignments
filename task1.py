...
def retrieve(label, feat_space, K):
  ...
  *_, feat_db_idx, feat_db = config.FEAT_DESC_FUNCS[feat_space]
  ...
  similarity_metric, feat_db_idx, feat_db = config.FEAT_DESC_FUNCS[feat_space]
  ...
  helper.plot(None, None, top_k_imgs, top_k_ids, top_k_scores, K, feat_space, similarity_metric)

if __name__ == '__main__':
  while True:
    inp = helper.get_user_input(
      'K,feat_space,label',
      len(config.DATASET),
      len(set(config.DATASET.y))
    )
    retrieve(**inp)
