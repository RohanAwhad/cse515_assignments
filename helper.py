...
def load_semantics(fd: str) -> Tuple[Any, ...]:
    # Check if the 'latent_semantics' directory exists, if not, create it
    if not os.path.exists('latent_semantics'):
        os.makedirs('latent_semantics')

    file_path = f'latent_semantics/{fd}'
    
    # Check if the specific file exists within the directory
    if not os.path.exists(file_path):
        raise ValueError(f"File '{file_path}' does not exist. Please ensure you've generated the required latent semantics.")
