...
def get_label_vecs(feat_space: str):
  ...
  for label in range(max(config.DATASET.y) + 1):
    *_, feat_db_idx, feat_db = config.FEAT_DESC_FUNCS[feat_space]
    ...
    label_feat = torch.tensor(feat_db[label_feats, :].mean(0))  # pool feat_matrix into label vector
...
def retrieve(img_id, feat_space, K):
  ...
  # every feature extraction function seems to return a torch array so shouldn't need to convert
  ...
  for idx, score in zip(top_k_ids, top_k_scores): print(f'  - Label: {idx:3d} | {similarity_metric:20s}: {score:6.2f}')
  if isinstance(img_id, int): print(f'\n  - Original Label: {config.DATASET[img_id][1]:3d}')
  print('-'*50)
